...
def test_terminal_page_loads(serialhub_server):
    """测试终端页面加载"""
    base_url = serialhub_server["url"]
    resp = requests.get(f"{base_url}/terminal", timeout=5)
    assert resp.status_code == 200
    assert "text/html" in resp.headers.get("Content-Type", "")


def test_html_contains_xterm_references(page: Page, serialhub_server):
    """测试 HTML 包含 xterm 引用"""
    base_url = serialhub_server["url"]
    page.goto(f"{base_url}/terminal", wait_until="load")
    html = page.content()
...
def test_static_resources_load(serialhub_server):
    """测试静态资源加载"""
    base_url = serialhub_server["url"]
    resources = [
        "/static/xterm.css",
...
def test_xterm_css_mime_type(serialhub_server):
    """测试 xterm.css MIME 类型"""
    base_url = serialhub_server["url"]
    resp = requests.get(f"{base_url}/static/xterm.css", timeout=5)
    assert resp.status_code == 200
    content_type = resp.headers.get("Content-Type", "")
    assert "text/css" in content_type, f"实际 Content-Type: {content_type}"


def test_xterm_js_mime_type(serialhub_server):
    """测试 xterm.js MIME 类型"""
    base_url = serialhub_server["url"]
    resp = requests.get(f"{base_url}/static/xterm.min.js", timeout=5)
    assert resp.status_code == 200
    content_type = resp.headers.get("Content-Type", "")
    assert "javascript" in content_type, f"实际 Content-Type: {content_type}"


def test_no_console_errors(page: Page, serialhub_server):
    """测试无控制台错误"""
    base_url = serialhub_server["url"]
    errors = []
...
def test_terminal_object_exists(page: Page, serialhub_server):
    """测试 Terminal 对象存在"""
    base_url = serialhub_server["url"]
    page.goto(f"{base_url}/terminal", wait_until="load")
...
def test_websocket_connects(page: Page, serialhub_server):
    """测试 WebSocket 连接"""
    base_url = serialhub_server["url"]
    page.goto(f"{base_url}/terminal", wait_until="load")
